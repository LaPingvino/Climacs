...
(defun possibly-expand-abbrev (mark)
  (let ((buffer (buffer mark)))
    (when (and (not (beginning-of-buffer-p mark))
	       (constituentp (char-before mark)))
      (let ((offset1 (offset mark))
	    (offset2 (offset mark)))
	(loop until (zerop offset1)
	      while (constituentp (buffer-char buffer (1- offset1)))
	      do (decf offset1))
	(let ((expansion (expand-abbrev (buffer-string buffer offset1 offset2)
					(abbrev-expander buffer))))
	  (when expansion
	    (delete-buffer-text buffer offset1 (- offset2 offset1))
	    (insert-buffer-text buffer offset1 expansion)))))))
